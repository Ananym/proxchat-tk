...
public class AudioService : IDisposable
{
    ...
    /// The root cause of the recurring volume bug was that changing UI settings (peer volume,
    /// overall volume) would call ApplyVolumeSettings without distance information, causing
    /// the volume calculation to temporarily ignore distance attenuation (defaulting to 1.0f).
    /// 
    /// The fix ensures distance information is always passed from MainViewModel where it's
    /// available in PeerViewModel.Distance, preventing the volume calculation from being
    /// incorrect until the next position update.
    /// 
    /// Key changes:
    /// - SetPeerUiVolume now accepts optional distance parameter
    /// - SetOverallVolumeScaleWithDistances created for MainViewModel to use
    /// - MainViewModel passes current peer distances when changing volume settings
    /// - Clear documentation added to prevent reintroduction of this bug
    /// </summary>
